"""
audio_client.py — cliente de voz do Raspberry Pi (etapa d).
...
O Pi faz o mínimo: grava, manda, mostra. Transcrição e "cérebro" são do PC.
A síntese de voz (TTS) da resposta entra numa sub-etapa posterior; por
enquanto a resposta só é impressa.

Uso (a partir de src/):
    python -m rasp.audio_client [host] [porta]
